    ...
    public void onMessage (WebSocket webSocket, ConversationMessageNotification msg) {
        if (msg.senderPermissions.contains(GroupPermissions.BAN_USER)) { //for now don't let everyone create commands
            if (MongoConnection.isPersistanceEnabled()) {
                ...
                if (parts.length < 2) {
                    webSocket.sendMessage(msg.conversationID, "usage: " + Config.getBotTrigger() + "delcursecheck <author> <type(optional)>");
                } else {
                ...
                webSocket.sendMessage(msg.conversationID, "can not delete check as persistence is disabled");

            }
        } else {
            webSocket.sendMessage(msg.conversationID, "can not delete check as you don't have the necessary permissions");
        }
    }
